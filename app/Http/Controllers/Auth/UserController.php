    ...
    public function address_update(Request $request)
    {
        try {
            ...
            if ($old->orders()->exists()) {
                ...
                $new->wards = $request->wards + 0;
                $new->district = $request->district + 0;
                $new->city = $request->city + 0;
                $new->zip_code = $request->zipcode + 0;
                ...
                $old->wards = $request->wards + 0;
                $old->district = $request->district + 0;
                $old->city = $request->city + 0;
                $old->modiffed_by = "User " . $old->user->user_name;
                $old->zip_code = $request->zipcode + 0;
                $old->save();
            }
    ...
    public function address_add(Request $request)
    {
        try {
            ...
            $new = UserAddress::create([
                ...
                "wards" => $request->wards + 0,
                "district" => $request->district + 0,
                "city" => $request->city + 0,
                "zip_code" => $request->zipcode + 0
            ]);
            //Check duplicate
