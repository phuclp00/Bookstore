...
class ShopController extends Controller
{
    ...
    public function show_product(Request $request)
    {
        ...
        //View count
        $current_ip = Cache::get('client.ip.' . $request->ip() . ".product." . $product->book_name);
        if ($current_ip == null) {
            $view = Redis::incr('product:' . $product->book_id . ':view');
            Cache::add('client.ip.' . $request->ip() . ".product." . $product->book_name, $request->ip(), now()->addSeconds(30));
        } else {
            $view = Redis::get('product:' . $product->book_id . ':view');
        }
        //Show rate avg
        $product->get_rate_point = $product->get_rate_point();
        //Related
