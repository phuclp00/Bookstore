    ...
    public function check_quantity(Request $request)
    {
        ...
        foreach ($cart as $value) {
            ...
            if ($value["quantity"] <= 0) {
                return \response([
                    'status' => 'danger',
                    'mess' => "Sản phẩm '" . $book->book_name . "' không được nhỏ hơn 0. Vui lòng kiểm tra lại!"
                ]);
            }
        ...
        try {
            ...
            foreach ($cart as $key => $item) {
                ...
                if ($promotion) {
                    ...
                    $books[$key]->load(['promotion' => function ($query) {
                        $query->withTrashed();
                    }]);
                    if ($books[$key]->promotion) {
                        if ($books[$key]->promottion->deleted_at) {
                            $mess .= " Chương trình khuyến mãi dành cho Sản phẩm '" . $books[$key]->book_name . "hiện đang tạm dừng! Liên hệ bộ phận CSKH để biết thêm chi tiết !";
                        }
                    }
                }
            }
            return \response([
                'status' => 'success',
                'data' => $books,
                'mess' => 'Tải danh sách sản phẩm trong giỏ hàng thành công ! ' . $mess
            ]);
        } catch (\Throwable $th) {
            return \response([
                'status' => 'danger',
