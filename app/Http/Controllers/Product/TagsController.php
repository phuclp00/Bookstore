...
class TagsController extends Controller
{
    ...
    public function remove_book($tags_id, $book_id)
    {
        try {
            DB::beginTransaction();
            $tag = TagsModel::find($tags_id);
            if ($tag) {
                $tag->books()->detach($book_id);
            } else {
                return response(['status' => 'danger', 'mess' => 'Not found this tags ! Try again  !']);
            }
            DB::commit();
            $list = TagsModel::all()->load([
                'books.thumb',
                'books.category',
                'books.tags',
                'books.format',
            ...
            return \response(['status' => 'success' , 'data' => $list, 'mess' => 'Remove book from tags was success !']);
        } catch (\Throwable $th) {
            DB::rollBack();
            return \response(['status' => 'danger', 'mess' => 'Remove tags was failed with error :' . $th->getMessage()]);
        }
    }
}
