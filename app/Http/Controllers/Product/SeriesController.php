...
use App\Http\Requests\Series\Series_Update;
use App\Http\Requests\Series\SeriesRequest;
use App\Models\BookSeries as Series;
...
{
    public function edit(Series_Update $request)
    {
        try {
            DB::beginTransaction();
            $data = Series::withTrashed()->where('id', $request->id)->first();
            $file =  new ImagesController(250, 200, $request->id);
            $data->name = $request->name;
            $data->description = $request->content;
            if ($request->img) {
                $data->image = $file->update($request->img, $data->image, "series");
            }
    ...
    public function add(SeriesRequest $request)
    {
        ...
        try {
            ...
            $product = Series::create([
                ...
                'image' => $request->img != null ? $file->store($request->img, "series") : null,
            ...
            $request->session()->flash('infor_warning', '<div class="alert alert-danger" style="text-align: center;font-size: x-large;font-family: fangsong;"> Add ' . $request->name . 'Failed with ' . $th->getMessage() . ',Try Again !! </div>');
            DB::rollBack();
            return \redirect()->back();
        }
    }
    public function delete(Request $request)
    {
        ...
        try {
            DB::beginTransaction();
            $data = Series::where('id', $request->id)->with('books')->first();
            if ($data->books()->exists() == false) {
                ...
                $data->save();
                $data->delete();
                $request->session()->flash('infor_mess', '<div class="alert alert-success" style="text-align: center;font-size: x-large;font-family: fangsong;"> The series ' . $request->name . ' has been moved to the "Restore List" category!! </div>');
            }
            DB::commit();
            return redirect()->route("admin.series");
        } catch (Exception $e) {
